...
针对 Embedded Linux 目标机，环境搭建走 **环境解耦与二进制对齐** 三阶段流程（详见 `EMBEDDED_CROSS_COMPILE_SKILL.md`）：

```text
Grill 共识 → ① apt 交叉工具链(国内源) → ② rsync sync_sysroot → ③ Docker COPY sysroot → CMake 构建 → 上板 ldd 验证
```

| 原则 | 说明 |
|------|------|
| 工具链/运行库分离 | apt 只装 `gcc-aarch64-linux-gnu`；ARM64 库只来自板端 rsync |
| 禁止 apt ARM64 库 | Dockerfile 不得 `apt install :arm64` 包替代 sysroot |
| Sysroot 精准同步 | `scripts/sync_sysroot.sh` 同步 `/lib/`、`/usr/lib/`；禁止全量 rootfs |
| CMake 隔离宿主机 | `CMAKE_SYSROOT` + `-DCMAKE_FIND_ROOT_PATH=/usr/aarch64-linux-gnu` |
| ldd 驱动补齐 | `not found` → 单库 rsync，不回退 apt |
| 双网卡 | 外网装工具链，直连网卡 rsync/scp 开发板 |
...
```text
嵌入式新功能：Grill → [可选 cross-env] → TDD → scp 部署 → 板上 ldd 验证
```
...
| `EMBEDDED_CROSS_COMPILE_SKILL.md` | 嵌入式交叉编译 Skill（环境解耦 · 二进制对齐） |
